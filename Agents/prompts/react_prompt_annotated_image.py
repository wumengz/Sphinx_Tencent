...
PREVIOUS ACTION: None""",
	...
	"template": """
OBSERVATION: The observation of the current app screen is the image provided below, with each actionable element annotated with a bounding box and an index inside the bounding box. The color of the bounding box and the color of the background of the index indicates its action type. """
+
", ".join([f"{color} indicates {action} action" for action, color in color_action_mapping.items()]) + "."
+
"""
TASK: {instruction}
