...
class Device:
    ...
    def run(self):
        ...
        for i, task in enumerate(self.tasks):
            ...
            docs_dir = Path("./docs") / llm / app
            print("=" * 20, self.port, "running", str(i)+"-th", "of", len(self.tasks), "tasks", "=" * 20, "\n",
                  "Running task", task["id"], "on app", task["app"], "with llm",
                  task["llm"], "and observation mode", task["observation_mode"], "\n",
                  "Save to dir:", trace_dir / str(task["id"]) / task["app"])
            cmdline = ["python", "run_appagent.py", "--id", str(id), "--app", app, "--llm", llm, "--port",
                       self.port, "--trace_dir", trace_dir, "--docs_dir", docs_dir]
            if tell_skill:
    ...
    for task in task_info:
        ...
        for app in task["apps"]:
            for config in configs:
                ...
                id = task["id"]
                trace_dir = Path("./trace_appagent_round1") / llm
                meta_path = trace_dir / observation_mode / str(id) / app / "meta.json"
                action_path = trace_dir / observation_mode / str(id) / app / "actions.json"
                ...
                if meta_path.exists():
                    ...
                    if meta["error"].startswith("RetryError"):
                        ...
                        meta_path.unlink()
                        pass
                    elif meta["error"].startswith("device"):
                        meta_path.unlink()
                        pass
