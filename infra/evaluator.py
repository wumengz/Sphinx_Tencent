...
class LastActionEvaluator(ActionEvaluator):
    ...
    def __init__(self, check_rules: Dict[str, str], check_type: str = "equal"):
        ...
        self.check_rules = check_rules
        self.check_type = check_type
        if ("action_type" in self.check_rules and self.check_rules["action_type"] == "input") or ("message" in self.check_rules):
            self.check_rules = dict()
            if "action_type" in check_rules:
                self.check_rules["action_type"] = check_rules["action_type"]
            if "message" in check_rules:
                self.check_rules["message"] = check_rules["message"]
...
class FindActionEvaluator(ActionEvaluator):
    ...
    def __init__(self, match_rules: Dict[str, str], check_rules: Dict[str, str], match_type: str = "equal", check_type: str = "equal"):
        ...
        self.match_rules = match_rules
        self.check_rules = check_rules
        if ("action_type" in self.match_rules and self.match_rules["action_type"] == "input") or ("message" in self.match_rules):
            self.match_rules = dict()
            if "action_type" in match_rules:
                self.match_rules["action_type"] = match_rules["action_type"]
            if "message" in match_rules:
                self.match_rules["message"] = match_rules["message"]
        if ("action_type" in self.check_rules and self.check_rules["action_type"] == "input") or ("message" in self.check_rules):
            self.check_rules = dict()
            if "action_type" in check_rules:
                self.check_rules["action_type"] = check_rules["action_type"]
...
class FindElementByActionEvaluator(ElementEvaluator, ActionEvaluator):
    ...
    def __init__(self, action_match_rules: Dict[str, str], element_match_rules: Dict[str, str], element_check_rules: Dict[str, str], action_match_type: str = "equal", element_match_type: str = "equal", element_check_type: str = "equal"):
        ...
        if ("action_type" in self.match_rules and self.match_rules["action_type"] == "input") or ("message" in self.match_rules):
            self.match_rules = dict()
            if "action_type" in action_match_rules:
                self.match_rules["action_type"] = action_match_rules["action_type"]
            if "message" in action_match_rules:
                self.match_rules["message"] = action_match_rules["message"]
